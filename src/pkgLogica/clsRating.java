...
public class clsRating {
    ...
    public static int rateAttack() {
        int counter=0;
        int tempPositionC=clsMovimientos.kingPositionC;
        for (int i=0;i<64;i++) {
            switch (clsMovimientos.chessBoard[i/8][i%8]) {
                case "P": {clsMovimientos.kingPositionC=i; if (!clsMovimientos.kingSafe()) {counter-=64;}}
                    break;
                case "R": {clsMovimientos.kingPositionC=i; if (!clsMovimientos.kingSafe()) {counter-=500;}}
                    break;
                case "K": {clsMovimientos.kingPositionC=i; if (!clsMovimientos.kingSafe()) {counter-=300;}}
                    break;
                case "B": {clsMovimientos.kingPositionC=i; if (!clsMovimientos.kingSafe()) {counter-=300;}}
                    break;
                case "Q": {clsMovimientos.kingPositionC=i; if (!clsMovimientos.kingSafe()) {counter-=900;}}
                    break;
            }
        }
        clsMovimientos.kingPositionC=tempPositionC;
        if (!clsMovimientos.kingSafe()) {counter-=200;}
        return counter/2;
    }
    ...
    public static int rateMoveablitly(int listLength, int depth, int material) {
        ...
        if (listLength==0) {//current side is in checkmate or stalemate
            if (!clsMovimientos.kingSafe()) {//if checkmate
        ...
        for (int i=0;i<64;i++) {
            switch (clsMovimientos.chessBoard[i/8][i%8]) {
                ...
                case "Q": counter+=queenBoard[i/8][i%8];
                    break;
                case "A": if (material>=1750) {counter+=kingMidBoard[i/8][i%8]; counter+=clsMovimientos.posibleA(clsMovimientos.kingPositionC).length()*10;} else
                {counter+=kingEndBoard[i/8][i%8]; counter+=clsMovimientos.posibleA(clsMovimientos.kingPositionC).length()*30;}
                    break;
            }
        }
        return counter;
